...
const wallpaper_1 = require("../types/wallpaper");
...
function currentMinuteBucket(date = new Date()) {
...
function resetWindowsIfNeeded(source) {
...
function remaining(limit, used, reserveRatio) {
...
function recordUsage(source, result) {
    ...
    const state = runtimeState[source];
    const limits = config_1.SOURCE_LIMITS[source];
...
function getQuotaReport() {
    return wallpaper_1.REMOTE_WALLPAPER_SOURCES.reduce((report, source) => {
        const state = runtimeState[source];
        const remainingQuota = getRemaining(source);
        const primaryRemaining = remainingQuota.minute !== "infinite"
        ...
        report[source] = {
            ...
            configured: (0, config_1.isSourceConfigured)(source),
