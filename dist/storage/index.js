...
function getFavorites() {
    ...
    const rows = db.prepare("SELECT payload FROM favorites ORDER BY updated_at DESC").all();
    return rows.map((row) => (0, persistence_1.safeJsonParse)(row.payload, {})).filter((item) => Boolean(item.id));
}
...
function isFavorite(id) {
    ...
    const row = db.prepare("SELECT 1 AS found FROM favorites WHERE id = ? LIMIT 1").get(id);
...
function getDownloadHistory() {
    ...
    const rows = db.prepare("SELECT payload FROM download_history ORDER BY downloaded_at DESC").all();
    return rows.map((row) => (0, persistence_1.safeJsonParse)(row.payload, {})).filter((item) => Boolean(item.id));
}
...
function getSearchHistory() {
    ...
    const rows = db.prepare("SELECT query FROM search_history ORDER BY created_at DESC").all();
...
function getSubscriptionState() {
    return readAppState("subscriptionState", "free");
}
...
function consumeFirstLaunch() {
    const firstLaunch = readAppState("firstLaunch", true);
