...
import { buildWallpaper, createSvgDataUrl } from "../utils";

type OfflineBundleSeed = {
  id: string;
  category: string;
  title: string;
  description: string;
...
const OFFLINE_BUNDLE_SEEDS: readonly OfflineBundleSeed[] = [
...
function getBundleSearchText(wallpaper: Wallpaper): string {
  return [
    wallpaper.category,
...
export function getBundledOfflineWallpapers(): Wallpaper[] {
  if (!cachedBundle) {
    cachedBundle = OFFLINE_BUNDLE_SEEDS.map((seed, index) => {
      const dataUrl = createSvgDataUrl(seed.title, seed.colors[0], seed.colors[1]);
...
export function searchBundledOfflineWallpapers(
